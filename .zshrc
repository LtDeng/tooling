...
# alias gdlb=`git fetch -p && for branch in $(git for-each-ref --format '%(refname) %(upstream:track)' refs/heads | awk '$2 == "[gone]" {sub("refs/heads/", "", $1); print $1}'); do git branch -D $branch; done`
...
alias p3.8=/opt/homebrew/bin/python3.8
...
#   git push https://"$uservar"@github.com/B_user_name/project.git
# }

ltdengGit () {
  GIT_SSH_COMMAND="ssh -i ~/.ssh/id_ed25519_ltdeng" $1
}
